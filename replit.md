...
*   **New `meta_source` value: `'naturalized'`.** Schema comment in `shared/schema.ts` updated; column is `varchar(16)` so no migration needed. Allowed values are now `'llm' | 'naturalized' | 'fallback' | 'manual'`.
*   **Wired into the live Pair pipeline** (`lib/newsroom/pairAgentOrchestrator.ts`): when the copywriter's own meta fails the brand+city contains-check (Tier-1 reject), the orchestrator now computes the formula (Tier-2) AND passes it through `naturalizeMeta`. If the polish passes the guards we ship it as `metaSource='naturalized'`; otherwise we ship the formula unchanged as `metaSource='fallback'` (legacy behavior preserved). Tier-1 path (LLM-emitted meta that already passes brand+city) is unchanged — those articles already had naturally-blended meta.
*   **Backfill script gets `--naturalize` flag** (`scripts/backfill-tableicity-meta.ts`). Combined with `--force --confirm --prod`, this re-stamps every published Tableicity article through the naturalizer. Per-row console line shows naturalized vs. fallback; tail summary shows total OpenAI cost. Idempotency rule unchanged: locked rows still need `--force` to overwrite. Pure meta-only UPDATE — does not touch `updated_at` / `date_modified` / `status` / `slug` (sitemap last-mod stays content-driven, not SEO-meta-driven).
*   **Why this is not "glue":** the formula is the safety net (still fires when the naturalizer fails any guard); the naturalizer is the only thing that produces the `'naturalized'` source value. One module, one prompt, one set of guards, used identically by the live pipeline and the backfill — no per-tenant prompt tweaks, no per-article hand-edits.
