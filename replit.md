...
*   **City-meta sweep (auto-fill)**: runs automatically on every newsroom-scheduler heartbeat (no setup). Optional dedicated cadence: point a Replit Scheduled Deployment at `node scripts/city-meta-sweep-tick.mjs` (e.g. every 60 min); honors `CITY_META_SWEEP_LIMIT` (default 25). Manual probe: `curl -H "x-cron-secret: $CRON_SECRET" "$BASE/api/cron/city-meta-sweeper?dryRun=1"` (free count).
...
*   **City Meta Generation (SEO)**: Per-city meta title/description are generated **write-time** by an LLM (`lib/cities/cityMetaGenerator.ts`), RAG-grounded on the tenant's Haylo **truth document** (`tenants.default_haylo_article_id`). Description band 130–165 (brand once, in the closing sentence, never in the first 40 chars); **Title 55–65, city verbatim, brand-free, and state-code-free** — city titles are city-only ("Austin", never "Austin, TX"); the shared article-title gate still allows the state. Triggered three ways: per-row from the admin Cities listing (sparkle button → `app/api/admin/cities/[id]/generate-meta`), in bulk via `scripts/backfill-city-meta.ts`, or **automatically by a reconciling CRON sweeper** (`app/api/cron/city-meta-sweeper` → `lib/cities/cityMetaSweeper.ts`) that mirrors the newsroom scheduler — once a tenant has a truth doc, the sweeper drains its cities still missing meta over successive ticks (CRON_SECRET-gated; `?dryRun=1` counts free; per-tick `?limit=` budget, default 25), so "meta fills itself" and the sparkle is now an override, not the only path. The sweeper reuses the exact one-city eligibility whitelist + TOCTOU-safe forward-only write and does **not** auto-designate truth docs. **Auto-trigger (zero human setup):** the sweep rides along on every **newsroom-scheduler heartbeat** — `app/api/cron/newsroom-scheduler/route.ts` runs the article tick first (fully committed), then calls `runCityMetaSweep` with a modest per-tick budget (`CITY_META_SWEEP_PER_TICK=10`) inside its own try/catch so a sweep failure can never break publishing. No separate schedule needed. When no cities are eligible the ride-along is ~2 cheap COUNT queries/tenant. An optional dedicated cadence (e.g. a Replit Scheduled Deployment every 60 min) can run `node scripts/city-meta-sweep-tick.mjs` instead/in addition — overlapping runs are safe (in-flight guard + per-row TOCTOU). Forward-only; `meta_locked_at` rows are never overwritten. **Full engineering history + restore point: `John/Scaffolding/Meta_Desc_Title.md` (§G7) + `John/Scaffolding/Regeneration.md` (revision-controlled, currently r4).**
