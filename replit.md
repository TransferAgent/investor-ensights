...
*   **City Meta Generation (SEO)**: Per-city meta title/description are generated **write-time** by an LLM (`lib/cities/cityMetaGenerator.ts`), RAG-grounded on the tenant's Haylo **truth document** (`tenants.default_haylo_article_id`). Description band 130–165 (brand once, in the closing sentence, never in the first 40 chars); **Title 55–65, city verbatim, brand-free, and state-code-free** — city titles are city-only ("Austin", never "Austin, TX"); the shared article-title gate still allows the state. Triggered per-row from the admin Cities listing (sparkle button → `app/api/admin/cities/[id]/generate-meta`) or in bulk via `scripts/backfill-city-meta.ts`. Forward-only; `meta_locked_at` rows are never overwritten. **Full engineering history + restore point: `John/Scaffolding/Meta_Desc_Title.md` + `John/Scaffolding/Regeneration.md` (revision-controlled, currently r3).**
