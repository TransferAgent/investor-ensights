...
| **MT-3** | **Tableicity Data Move (THE DANGEROUS ONE)** | App Builder | Closed MT-2, fresh `John/Dump_MT3_Pre.dump` | Per-tenant data copied from `public.*` → `tenant_<slug>.*` inside ONE transaction with COUNT verification, write-freeze (LOCK TABLE EXCLUSIVE), in-transaction provisioning (no atomicity gap), advisory lock (no parallel runs), statement+lock timeouts, composite-FK guard, strict registry insert. `tenants` row inserted with brand fields. `city_slug_registry` populated. Public copies left in place (dormant) for one gate as rollback safety net. **CLOSED ON DEV 2026-05-09.** Pre-gate dump: `John/Dump_MT3_Pre.dump` (1.8MB). | DEV: All 5 verifier checks PASS (568 rows mirrored across 24 tables, 14 FKs reconstructed, 18 city slugs claimed, public.* intact). Dev sitemap = 18 URLs unchanged (proves end-to-end routing through tenant_tableicity). Architect re-review PASS after 3 hardening fixes (in-tx provisioning, EXCLUSIVE lock write-freeze, strict registry). 289 LOC. **PROD execution pending Conductor go**: requires `bash scripts/push-schema-to-prod.sh` (5 new global tables) → `DATABASE_URL=$PROD_DATABASE_URL npx tsx scripts/mt3-move-tableicity.ts` (dry-run) → `--confirm` → verify → redeploy. Post-prod expected: sitemap = 84, 80 articles + 340 cities + 182 haylo all visible. | MT-4 |
| **MT-4 (SIMPLIFIED)** | **Auth Refactor (admin-creates-admin, no MFA, no public self-serve)** | App Builder | Closed MT-3 | **Conductor scope cut on 2026-05-09:** dropped MFA, dropped public "Create one" link, dropped `/login/set-password` + `/login/verify` pages, dropped `email_verifications` writes (table preserved for future). Shipped: `POST /api/admin/login` authenticates email+password against `public.users`, looks up tenant from `tenant_members`, issues JWT cookie carrying `{userId, email, tenantSlug}`. `GET /api/admin/me` returns user + tenant. `POST /api/admin/users` (admin-only) atomically creates user + tenant_member binding; tenant picker UI lets the creating admin select an existing tenant OR create a new tenant inline (provisions `tenant_<slug>` schema + inserts `tenants` row + binds user, all in one PG transaction). `GET /api/admin/tenants` powers the picker. Login UI rebuilt per D7 v1.1 (form-left-white + brand-right-black `iE`). `scripts/mt4-migrate-admin-users.ts` (idempotent, dry-run/--confirm) backfilled existing `admin_users` → `users` preserving UUIDs (so `admin_audit_log.admin_id` FK still satisfied) + bound to `tenant_members(tableicity, tenant_admin)`. **CLOSED ON DEV 2026-05-09 alongside MT-6.** | DEV verifier: (a) abc17@gmail.com migrated 1→1 (users + tenant_members); (b) login flow returns 401 on bad pw, issues cookie on success; (c) tenant picker on Add User form lists `tableicity`; (d) "+ Create new tenant" inline form provisions schema + binds user atomically; (e) sitemap canary = 18 URLs unchanged; (f) all `/api/admin/*` return 401 without cookie; (g) typecheck clean for all new/changed files (pre-existing campaigns errors unrelated). LOC: ~700 (over 300/gate budget — Conductor accepted; combined MT-4+MT-6). | MT-5 (visual polish), MT-7 |
...
| **MT-6 (SHIPPED INSIDE MT-4)** | **Session → Tenant Context (silo enforcement)** | App Builder | Closed MT-3 | **Architecture pivot 2026-05-09:** instead of mass-refactoring 63 admin route files to wrap every handler in `withTenantAsync(session.tenantSlug, ...)`, ship a single Proxy at the storage layer. `lib/storage.ts` exports `storage` wrapped in `Proxy({ get: m => withSessionTenant(() => m.apply(...)) })`. `lib/tenant/session-tenant.ts` reads admin cookie + decodes tenantSlug + verifies tenant exists (60s cache) + runs the storage call inside `withTenantAsync(slug, fn)`. Gated by `x-ie-admin-context: 1` header set by `middleware.ts` on `/admin/*` and `/api/admin/*` paths only — public routes never have their tenant flipped by a logged-in admin's cookie. Two-line `lib/auth-middleware.ts` `withAdminAuth(handler)` helper available for routes that touch `db` directly (login, users/[id], me, tenants). Smoke test: Conductor creates `demo` tenant via admin form, signs in with demo user, sees zero articles/cities/haylo. **CLOSED ON DEV 2026-05-09 alongside MT-4.** | (a) Sitemap canary = 18 URLs unchanged (public routes still hit `tableicity` via env default); (b) tableicity admin sees 18 cities; (c) cross-tenant isolation: demo admin cannot read tableicity rows via storage; (d) deleted/missing tenant in cookie → falls through to env default (no privilege escalation); (e) cookie peek skipped on public paths (verified via header gating). **Known limitation:** JWT staleness for deleted USERS is up to 24h (cookie TTL); accepted tradeoff. | MT-5 (UI polish), MT-7 |
...
- **MT-2:** **CLOSED 2026-05-09.** Platform tables + provisioner shipped (302 LOC). Self-contained verifier passes 6/6. Sequencing Rule added.
- **MT-3:** **CLOSED ON DEV 2026-05-09.** Move script + verifier shipped (289 LOC). Dev: 568 rows mirrored, 14 FKs reconstructed, 18 city slugs claimed, sitemap unchanged at 18, public.* intact as rollback safety net. Architect re-review PASS after 3 hardening fixes. **PROD execution still pending Conductor go** (see DoD column for sequence). Once PROD runs, deploy ban lifts.
- **MT-4 (simplified) + MT-6:** **CLOSED ON DEV 2026-05-09.** Combined into one ship at Conductor request (no MFA, no public self-serve, silo via storage Proxy). PROD activation requires running PROD MT-3 first.
- **MT-5, MT-7 through MT-9:** Not yet open.
...
| v1.1 | 2026-05-09 | **MT-3 CLOSED ON DEV.** `scripts/mt3-move-tableicity.ts` + `scripts/mt3-verify.ts` shipped. One-transaction move with: in-transaction provisioning (closed atomicity gap), EXCLUSIVE write-freeze on all 24 public per-tenant tables, statement+lock timeouts, advisory lock (5318) for migration mutex, composite-FK guard, strict registry insert (no DO NOTHING), idempotency gate. Dev: 568 rows mirrored, 14 FKs reconstructed, 18 city slugs claimed, sitemap stayed at 18 (proving tenant-aware client now routes reads through tenant_tableicity). Architect first review FAIL (3 blockers); re-review PASS after fixes. 289 LOC. Pre-gate dump `John/Dump_MT3_Pre.dump` (1.8MB) captured. PROD execution pending Conductor go. | App Builder + Architect (Conductor sign-off pending; PROD execution pending) |
