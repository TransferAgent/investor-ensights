...
| Status | **RETIRED & CLOSED 2026-06-06** (Conductor: "100% accepted and 100% landed") — the Article meta subsystem was rebuilt into the clean `lib/newsroom/articleMetaGenerator.ts` (naturalizer + formula glue deleted); see r10. Historical record; live code is the source of truth. Kept in-folder per the forward-only rule. |
...
| r8 | 2026-06-06 | Agent | **ARTICLE naturalizer: stronger model + 3-shot + description-first (Conductor).** Article path only; **no band change** (desc 250–300, title 55–65 unchanged); auto-publish kept; body/copywriter untouched. Three changes, all in `lib/newsroom/metaNaturalizer.ts`: (1) **Model `gpt-4.1-mini` → `gpt-4.1` (full)** — the SERP snippet is the customer's first impression and mini couldn't reliably hit the 250–300 band (the same lesson that moved City description to full 4.1); `costFor()` repriced to $2/$8 per M. (2) **Two-shot → three-shot** — `MAX_ATTEMPTS=3`, temperature ladder `0.6 → 0.3 → 0.2`, the rejection reason is fed back after every non-final attempt; formula stays the never-throw safety net. (3) **Description-first ordering** (Conductor flipped the old title-first logic) — the system prompt now authors the DESCRIPTION first (the 80/20 RAG pain point) and DERIVES the title's hook from it; JSON shape reordered to `{ "description", "title" }`; retry hint reworded description-first. Model is internal to the module, so the live orchestrator AND the `--naturalize` backfill both inherit it. See §1 MT-4.14.1. |
| r9 | 2026-06-06 | Agent | **ARTICLE naturalizer: salvage over-length descriptions instead of formula fallback (Conductor).** Article naturalizer only; **no band / model / contract change** (desc 250–300, title 55–65, `gpt-4.1`, 3-shot, description-first all unchanged). New private helper `salvageTooLongDescription()` in `lib/newsroom/metaNaturalizer.ts`: when all 3 shots fail AND the *only* failure was `desc-too-long`, it brute-forces whole-sentence subsets of the rejected LLM copy (always keeping the brand "accent" closing sentence) for the combination that lands FULLEST inside 250–300 **and** passes the full `metaDescriptionAcceptable` guard, then re-checks via `validateOrNull` before shipping as `meta_source='naturalized'`. Whole sentences only ⇒ always clean prose; city + brand preserved; brand never in the lead 40 chars. Falls through to the formula unchanged when nothing fits (never-throw + forward-only intact). **Why:** PROD diagnosis showed wordy Haylo topics (e.g. the cap-table essay) push the LLM past 300 on all 3 shots → formula glue; salvage recovers LLM voice at **no extra API cost** (reuses the already-paid attempt). Proven: synthetic 441-char desc → 296 in-band PASS; 6 live PROD `texitie` pairs all `naturalized`. |
| r10 | 2026-06-06 | Agent | **RETIRED & CLOSED (Conductor: "100% accepted and 100% landed").** The Article meta subsystem was subsequently rebuilt into ONE clean LLM generator (`lib/newsroom/articleMetaGenerator.ts`): all of the naturalizer / formula glue described above — `metaNaturalizer.ts`, `buildMetaTitle` / `buildMetaDescription`, `META_LIMITS`, the brand-lead guard — was DELETED. New Article contract: title ≤65; description ≤320 with the brand injected AS the solution to the article's problem; on gate-fail the engine flags `needs-meta` and ships NO meta (never glue). The **City** meta path is unchanged. PROD-accepted. This document is RETIRED as a historical record — the live code is the source of truth. Kept in-folder per the forward-only rule. |
...
- **Haylo read excerpt 1000 → 4000 chars.** BOTH article naturalizer call sites now pass `4000` to `hayloBodyExcerptFromHtml` — the live orchestrator (`pairAgentOrchestrator.ts`) and the `--naturalize` backfill (`scripts/backfill-tableicity-meta.ts`) — so the naturalizer grounds its angle on a much larger plain-text slice. The helper's own default stays `1000` (the City truth-doc generator already passed `4000` explicitly and is unchanged).
...
### MT-4.14.1 — article naturalizer: stronger model, 3-shot, description-first (r8, 2026-06-06)
Conductor decision, **Article naturalizer only** (`lib/newsroom/metaNaturalizer.ts`). Bands unchanged from MT-4.14; auto-publish kept; **the body / copywriter pipeline is never touched** (Conductor standing rule — meta title + meta description only). Diagnosis that drove it: a PROD spot-check (`texitie-allentown-pa-…`) showed the live description was the deterministic **formula** (`…Textitie helps Allentown founders.`) — i.e. the LLM had failed its guards and we shipped glue. Root cause: the 250–300 band is a tight needle and **`gpt-4.1-mini` can't reliably count to it** (same finding that put City description on full `gpt-4.1`), and two swings wasn't enough headroom.
- **Model `gpt-4.1-mini` → `gpt-4.1` (full).** The naturalizer is the customer's first SERP impression, so it now runs on the strong model. Cost ~$0.004/article (worst case 3 attempts ~$0.012) — trivial. `costFor()` repriced to $2 / $8 per M.
- **Two-shot → three-shot.** `MAX_ATTEMPTS=3`, temperature ladder `0.6 → 0.3 → 0.2`; the machine-readable rejection reason is fed back after each non-final attempt. Falls open to the formula only if **all three** fail (still never throws).
- **Description-first ordering.** The Conductor flipped the old title-first framing: the description carries the substance (the 80% RAG pain point + 20% brand), so the prompt now writes the **DESCRIPTION first** and **derives the title's hook from it** ("must echo the same pain point, not a new subject"). JSON shape reordered to `{ "description", "title" }`; the retry hint leads with the description contract too.
- **Inheritance:** because the model + loop live inside the module, both the **live Pair orchestrator** (Tier-2.5) and the **`--naturalize` backfill** pick this up with no call-site change.
- **Unchanged:** description band (250–300), title band (55/65), the state-ban, the never-throw / forward-only rules, and the entire **City** pipeline (City description was already on full `gpt-4.1` and already description-first).

**Net article-meta rules as of r8:** same bands/guards as r7; the naturalizer now runs **`gpt-4.1` (full), up to 3 attempts, description-first** and is the strong-model first impression on every routed article.
