...
| Status | **RETIRED & CLOSED 2026-06-06** (Conductor: "100% accepted and 100% landed") — superseded restore point; the snapshotted naturalizer / formula architecture was replaced by the clean `lib/newsroom/articleMetaGenerator.ts` (see r9). Historical record; live code is the source of truth. Kept in-folder per the forward-only rule. |
...
| r8 | 2026-06-06 | Agent | *(this change set; commit recorded at task close)* | **ARTICLE naturalizer over-length salvage** (companion to `Meta_Desc_Title.md` r9 / MT-4.14.2). Article naturalizer only; **City pipeline unchanged**; **bands + model + anchors unchanged from r7** (article desc 250–300, title 55–65, `gpt-4.1`, 3-shot, description-first) — §§2–5 numbers are identical, only the naturalizer's fallback path changed. `lib/newsroom/metaNaturalizer.ts`: new private `salvageTooLongDescription(desc, brand, cityName, minLen, maxLen)`. On all-shots-fail where the ONLY failure was `desc-too-long`, it brute-forces whole-sentence subsets (brand accent kept as the closing sentence) for the fullest in-band candidate that passes `metaDescriptionAcceptable`, then `validateOrNull` re-checks before shipping `naturalized`. No extra API cost (reuses the rejected attempt). Formula safety net + never-throw + forward-only intact. |
| r9 | 2026-06-06 | Agent | — | **RETIRED & CLOSED (Conductor: "100% accepted and 100% landed").** Superseded restore point: the Article meta subsystem snapshotted above (naturalizer, 3-shot, salvage, formula fallback, `META_LIMITS`) was rebuilt into ONE clean LLM generator (`lib/newsroom/articleMetaGenerator.ts`) with the naturalizer + formula glue DELETED (title ≤65; description ≤320 with the brand framed AS the solution; `needs-meta` on gate-fail, never glue). The numbers / anchors below no longer describe the live code. The **City** meta path is unchanged. This document is RETIRED as a historical record — the live code is the source of truth. Kept in-folder per the forward-only rule. |
