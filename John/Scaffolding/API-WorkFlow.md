...
| Status | **RETIRED & CLOSED 2026-06-06** (Conductor: "100% accepted and 100% landed") — historical record; live code is the source of truth. Kept in-folder per the forward-only rule. |
| Owner | Conductor |
...
| r6 | 2026-06-06 | Agent | **Incident + recovery — deleted essays won't re-pull (watermark, not a bug).** Conductor deleted 3 Texitie essays and expected re-Pull to restore them; it didn't, because the Pull is incremental (`halo_last_pulled_id` watermark) and delete doesn't rewind it. Diagnosed the gap (remote ids 205–207 missing; watermark 324). Recovery: rewound `public.tenants.halo_last_pulled_id` to 204 on PROD (no code change) so the next Pull re-fetches 205–324; dedup re-imports only the 3 missing (as drafts) and skips the rest. New §4c documents the mechanism + recovery recipe. Open design question logged in §8 #6 (should delete rewind the watermark / offer a "force re-pull"?). |
...
| r7 | 2026-06-06 | Agent | **RETIRED & CLOSED (Conductor: "100% accepted and 100% landed").** Ingestion workflow (Halo Pull → Haylo Library → Content Studio → Article) is verified and shipped: the topic-slug forward-fix at import + the `backfill-haylo-topic.ts` backfill + the `bulk-set-haylo-ready.ts` promotion all landed and were verified on PROD. The open product questions in §8 (auto-promote on import, a UI "Mark all Ready" button, delete→watermark rewind) are parked, not blocking. This document is RETIRED as a historical record — the live code is the source of truth. Kept in-folder per the forward-only rule. |
...
## 4c. Gotcha — the Pull is incremental; deleting from the Library does NOT rewind it

The "Pull from API" button is **incremental, not a full re-sync**. Each tenant carries a high-water mark `public.tenants.halo_last_pulled_id`; every pull calls Halo with `?since=<watermark>` and only ingests ids **above** it, then advances the watermark to the highest id seen (`pull-from-halo/route.ts` L55/L160/L165–170).

**Delete is a HARD delete** (`storage.deleteHayloArticle` → `db.delete`, no soft-delete flag), and it does **not** touch the watermark. So a deleted essay's remote id is now *below* the watermark → Halo never re-sends it → clicking Pull recovers nothing (and shows "already imported / nothing new" for the rows still present). The two dedup layers (remote-id, content-hash) both read live `haylo_articles`, so they correctly DON'T block a re-import of a truly-deleted row — the only blocker is the watermark.

**Recovery (no new code):** rewind that tenant's `halo_last_pulled_id` to just below the lowest deleted id, then click Pull. Dedup skips everything still present and re-imports only the gap. Re-imported essays arrive `draft` (with the r4-derived topic), so they then need promoting to Ready.

> **Incident 2026-06-06 (r6):** Conductor deleted 3 Texitie essays (Halo remote ids **205, 206, 207** — a clean gap between 204 and the contiguous 208–324). Watermark was 324, so Pull couldn't reach back. Fix: `UPDATE public.tenants SET halo_last_pulled_id=204 WHERE slug='texitie'` (PROD), then Conductor clicks Pull → 205–207 re-import as drafts; 208–324 + 203–204 skipped by remote-id dedup. Halo upstream still holds those essays (delete was local only).
...
6. **OPEN — Should deleting an essay let it come back on re-Pull?** Today the watermark is forward-only and delete doesn't rewind it (§4c), so recovering a deleted essay needs a manual watermark rewind. Options if this recurs: (a) leave as-is (manual rewind, documented); (b) a "Force full re-pull" button that runs Pull with `since=0` (dedup already makes this safe); (c) rewind the watermark automatically when an essay below it is deleted. (a) is zero-bloat and fine for rare events.
